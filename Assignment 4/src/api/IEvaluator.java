...
/**
 * An IEvaluator is a utility for evaluating groups of cards in card games based
 * on ranking of hands. Each evaluator represents a type of hand based on some
 * evaluation criteria, such as pairs of matching cards or consecutive runs. An
 * evaluator also has a ranking relative to other evaluators that might be used
 * in a given game. In traditional poker games, ranking of hands is based on
 * mathematical probability, but there is no particular requirement here other
 * than that different evaluators are constructed with different rankings.
 * 
 * Within a given ranking the hands are also ordered. The ordering is defined by
 * the Hand class. Each hand consists of an array of <em>main</em> cards and a
 * (possibly empty) array of <em>side cards</em>. The Hand class orders hands
 * within the same ranking by first lexicographically comparing the main cards,
 * and if they are the same, lexicographically comparing the side cards.
 * Therefore, it is up to the evaluator to correctly order the cards in these
 * groups. The side cards can always be sorted according to the Card class
 * compareTo method, which orders cards in descending order with higher cards
 * first. In most cases it is sufficient to sort the main cards using the Card
 * class; however, this may not work for particular evaluators. For example, in
 * a traditional full house there is a group of three cards and a group of two
 * cards, and they are ordered based on the group of three, even if the group of
 * two consists of cards of higher rank. Thus, when creating a Hand, the main
 * cards would have to be listed with the group of three first, e.g., [3 3 3 5
 * 5] should defeat [2 2 2 10 10].
 */
public interface IEvaluator {
	...
	/**
	 * Returns the number of cards in a hand. This value is generally defined by a
	 * game, and is not necessarily the same as the value returned by
	 * <code>cardsRequired</code> (main cards plus side cards).
	 * 
	 ...
	 * cards. If the indicated subset does not s!~
	 * atisfy the criteria for this
